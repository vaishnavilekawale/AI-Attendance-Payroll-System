...
def compute_payroll_amounts(
    ...
    paid_days, lop_days = compute_paid_and_lop_days(present_days, absent_days, half_days)
    ...
    half_day_deduction = round_money(half_days * (per_day_salary / 2))
...
#     basic_salary = round_money(employee.basic_salary)
#     per_day_salary = round_money(basic_salary / working_days) if working_days > 0 else 0.0
...
#     employee_pf_percentage = getattr(employee, 'employee_pf_percentage', 12.0) or 0.0
#     employee_pf = round_money((basic_salary * employee_pf_percentage) / 100)

#     employer_pf_percentage = getattr(employee, 'employer_pf_percentage', 12.0) or 0.0
#     employer_pf = round_money((basic_salary * employer_pf_percentage) / 100)

#     esic_percentage = getattr(employee, 'esic_percentage', 0.75) or 0.0
#     esic = round_money((gross_salary * esic_percentage) / 100)

#     tds_percentage = getattr(employee, 'tds_percentage', 0.0) or 0.0
#     tds = round_money((gross_salary * tds_percentage) / 100)
...
#     net_salary = round_money(gross_salary - total_deductions + overtime_bonus)
#     net_ctc = round_money(gross_salary + employer_pf + overtime_bonus)
...
#     def _get_working_days(self, start_date, end_date):
#         """Count calendar days from start_date through end_date (inclusive)."""
#         if end_date < start_date:
#             return 0
