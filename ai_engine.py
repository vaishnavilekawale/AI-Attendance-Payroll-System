...
class FaceRecognitionEngine:
    ...
    #     Uses dynamic tolerance from Settings if not provided
        
    #     OPTIMIZATIONS:
    #     - Uses cached employee images to avoid repeated disk I/O
    #     - Processes frame in memory without temporary file when possible
    #     - Single global instance to avoid repeated模型 loading
    #     """
    #     import time
    #     start_time = time.time()
    ...
    #         # Resize frame to 640x480 for faster processing (as requested)
    #         resized_frame = cv2.resize(
    #             frame,
    #             (640, 480)
    #         )
            
    #         # Save temp file for DeepFace (required by DeepFace API)
    #         success = cv2.imwrite(temp_path, resized_frame)
            
    #         if not success or not os.path.exists(temp_path):
    #             logger.error("Failed to write temp image for recognition")
    #             return []
            
    #         # Load temp image as numpy array for in-memory processing
    #         temp_image = cv2.imread(temp_path)
    #         if temp_image is None:
    #             logger.error("Temp image loading failed")
    #             return []
            
    #         bbox = None
    #         # Face detection with validation and retry logic
    #         # This prevents accepting the entire frame as a face when detection fails
    #         max_detection_attempts = 2  # Retry once if first detection fails
    ...
    #                     # Validate bbox: reject if it's the entire frame (indicates detection failure)
    #                     # Frame is 640x480, so reject if bbox covers most of the frame
    #                     bbox_area = candidate_bbox[2] * candidate_bbox[3]  # width * height
    #                     frame_area = 640 * 480  # 307200 pixels
                        
    #                     # If bbox covers more than 80% of frame, it's likely a detection failure
    #                     if bbox_area > 0.8 * frame_area:
    #                         logger.warning(f"Detection attempt {attempt + 1}: Invalid bbox (covers {bbox_area/frame_area*100:.1f}% of frame), retrying...")
    #                         if attempt < max_detection_attempts - 1:
    #                             continue  # Retry
    #                         else:
    #                             logger.warning("Face detection failed after retries - bbox covers entire frame")
    #                             bbox = None
    #                             break
    #                     else:
    #                         # Valid face detection
    #                         bbox = candidate_bbox
    #                         logger.debug(f"Valid face detected: bbox={bbox}, area={bbox_area/frame_area*100:.1f}% of frame")
    #                         break
    #                 else:
    #                     logger.warning(f"Detection attempt {attempt + 1}: No faces detected")
    #                     if attempt < max_detection_attempts - 1:
    #                         continue  # Retry
    #                     else:
    #                         bbox = None
    #                         break
    #             except Exception as e:
    #                 logger.debug(f"Face detection error on attempt {attempt + 1}: {e}")
    #                 if attempt < max_detection_attempts - 1:
    #                     continue  # Retry
    #                 else:
    #                     bbox = None
    ...
    #         # Select employees to check
    #         if target_employee_id:
    #             if target_employee_id in self.known_face_ids:
    #                 employees_to_check = [target_employee_id]
    #             else:
    #                 logger.warning(f"Employee not found in trained list: {target_employee_id}")
    #                 return []
    #         else:
    #             employees_to_check = self.known_face_ids
            
    #         # Compare images using cached employee images.
    #         # Early-exit: stop once a high-confidence match (distance < 0.23,
    #         # well inside the 0.3 tolerance) is found, so we skip the remaining
    #         # cached images and employees for a faster response.
    #         high_confidence_match = False
    #         for employee_id in employees_to_check:
    #             # Use cached images to avoid repeated disk I/O
    #             employee_images = get_employee_images_cached(employee_id)
                
    #             if not employee_images:
    #                 continue
                
    #             for idx, employee_image in enumerate(employee_images, 1):
    #                 try:
    #                     # Verify using in-memory numpy arrays (faster than file paths)
    ...
    #                         # High-confidence early exit: distance < 0.23 is
    #                         # a very strong match (well within 0.3 tolerance).
    #                         if distance < 0.23:
    #                             high_confidence_match = True
    #                             break
    #                 except Exception as e:
    #                     logger.debug(f"Verification error for employee {employee_id}: {e}")
    #                     continue

    #             # Stop checking remaining employees once a high-confidence
    #             # match has been secured in the inner image loop.
    ...
    #             logger.info(f"Face recognized: {name} (ID: {matched_employee_id}), confidence: {confidence:.2f}")
    #         else:
